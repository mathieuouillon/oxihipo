...
shipped code — for example, the binding pins pyo3 0.29 and therefore uses
`Python::detach` (the `allow_threads` → `detach` rename landed in pyo3 0.25), and features like
...
- **Composite/opaque banks** → read columnar through [`f.composite(bank)`](../python/reading.md#composite-banks), with fields named positionally (`f0`, `f1`, …) because the inline format string carries types but no names. *Two ways this differs from the sketch:* it is columnar rather than the raw `f.raw(bank, entry) -> bytes` escape hatch originally planned; and they are **not** excluded from `keys()` — `composite()` resolves the name through the dictionary just as `arrays()` does, so a composite bank is listed like any other and `keys()` cannot distinguish the two. A discovery call is still missing.
...
- **pyo3 features:** `["extension-module", "abi3-py313", "generate-import-lib"]` (pyo3 0.29; the `abi3-py313` floor has been available since pyo3 0.24). abi3 → **one `cp313-abi3` wheel per (OS, arch)**, matching `requires-python >= 3.13` exactly (that metadata is what enforces the minimum — pip refuses older). `generate-import-lib` lets the **Windows** wheel build interpreter-free (via `python3-dll-a`) — needed because `requires-python >= 3.13` leaves the CI runner with no interpreter maturin will accept, so it builds without one. `extension-module` drops the libpython link (manylinux-clean). rust-numpy is orthogonal to the CPython limited API — it resolves NumPy's C-API at import time through a capsule, so abi3 + numpy coexist.
